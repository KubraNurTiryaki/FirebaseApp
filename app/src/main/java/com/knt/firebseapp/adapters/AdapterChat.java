...
import com.knt.firebseapp.LoginActivity;
        ...
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot ds : snapshot.getChildren()) {
                    /*if you want to allow sender to delete only their message then compare sender value with current user's uid
                     * if they match that means its the message of sender that is trying to delete*/

                    if (ds.child("sender").getValue().equals(myUID)) {

                        /*We can do one of two things here
                         * 1) Remove the message from Chats                          -> Instagramdaki gibi
                         * 2) Set the value of message "This message was deleted..." -> WhatsApptaki gibi
